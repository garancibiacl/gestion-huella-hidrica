...
// Ejecutado por cron cada 3 minutos - SIEMPRE genera HTML v3 dinámicamente
// ============================================================================
...
async function fetchHazardReportData(
  ...
  const { data, error } = await supabaseClient
    .from('hazard_reports')
    .select(`
      id, description, faena, gerencia, proceso, actividad,
      due_date, critical_risk_name, verification_responsible_name,
      closing_responsible_name, created_at
    `)
    .eq('id', reportId)
    .single();
...
async function sendEmailWithResend(params: {
  ...
  try {
    console.log(`📧 Sending to ${params.to}: ${params.subject}`);
...
async function markAsSent(supabaseClient: any, recordId: string, messageId?: string): Promise<void> {
  ...
  if (error) console.error(`Error marking ${recordId} as sent:`, error);
}
...
async function markAsFailedOrRetry(
  ...
  if (updateError) console.error(`Error updating ${recordId}:`, updateError);
  
  if (!shouldRetry) {
    console.error(`❌ ${recordId} failed permanently after ${newAttempts} attempts`);
  } else {
    console.log(`⚠️ ${recordId} will retry (${newAttempts}/${MAX_ATTEMPTS})`);
  }
}

/**
 * Genera subject y HTML v3 para un registro
 */
async function generateEmailContent(
  ...
  if (normalizedEntityType === 'hazard_report') {
    ...
    return {
      ...
      html: generateEmailHtml(record.notification_type, payload, recipientName, ctaUrl),
    };
  }
  
  if (normalizedEntityType === 'pam_task') {
    const payload: EmailNotificationPayload = {
      type: record.notification_type,
      title: 'Tarea PAM',
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const result: ProcessResult = { processed: records.length, sent: 0, failed: 0, errors: [] };
    ...
    for (const record of records as OutboxRecord[]) {
      try {
        ...
        // 4.3 SIEMPRE generar HTML v3 dinámicamente (ignorar html_body de DB)
        ...
        if (sendResult.success) {
          ...
          result.sent++;
          console.log(`✅ Sent: ${record.id} → ${recipient.email}`);
        } else {
          await markAsFailedOrRetry(supabaseClient, record.id, record.attempts, sendResult.error || 'Unknown');
          result.failed++;
          result.errors.push(`${record.id}: ${sendResult.error}`);
        }
      } catch (error: any) {
        await markAsFailedOrRetry(supabaseClient, record.id, record.attempts, error.message || 'Processing error');
        result.failed++;
